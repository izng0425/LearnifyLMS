// src/pages/InstructorEditLessons.jsx
import React, { useState, useEffect } from "react";
import { useNavigate, useParams } from "react-router-dom";

/**
 * InstructorEditLessons
 */
export default function InstructorEditLessons() {
  const navigate = useNavigate();
  const { lessonId } = useParams(); // Mongo _id of the lesson in URL

  // Keep both the document _id and the human-friendly lessonId(code)
  const [docId, setDocId] = useState(""); // current document _id
  const [id, setId] = useState("");       // lessonId (e.g., W2)
  ...
  function handleReset() {
    // Reset to blank but keep "createdBy" from token so it's prefilled/visible
    setId("");
    setTitle("");
    setDescription("");
    setObjective("");
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5050/lessons/${lessonId}`, {
      ...
      .then((data) => {
        // Preserve document id for "self" checks
        ...
        // Safety: remove "self" from prerequisites if already present
  ...
  // ---- Load all lessons (for prerequisites list) ----------------------------
  ...
  async function handleUpdate() {
    ...
    // Safety before sending: make sure "self" is not present
          ...
          <div className="max-h-60 overflow-y-auto rounded-md border border-teal-200 divide-y divide-teal-100">
            {existingLessons.length === 0 ? (
              ...
              existingLessons.map((lesson) => {
                // Hide the current lesson itself from the checklist
                ...
                return (
                  <label key={lesson._id} className="flex items-center gap-2 p-2 hover:bg-slate-50">
                    <input
                      ...
                      checked={checked}
                      onChange={(e) => {
                        if (sameId(lesson._id, docId || lessonId)) return; // extra guard
                        if (e.target.checked) {
                          setPrereqIds((prev) =>
