...
export default function InstructorLessonsContent() {
  ...
  // ---- Helpers ----
  // Normalize ObjectId strings like ObjectId("...") → "..."
  ...
  const backHref = state?.backTo || "/instructor-lessons";

  return (
    <main className="mx-auto max-w-6xl px-4 py-8">
      {/* Header: keep airy — only title + Back (no chips or meta) */}
      ...
      {/* Single card with each field as its own block (like the Course page) */}
            ...
            <ul className="mt-2 list-disc pl-5">
              {prereqItems.map(({ id, title }, idx) => (
                <li key={id || idx}>
                  {id ? (
                    <Link
                      to={`/instructor-lessons-content/${id}`}
      ...
      {/* NOTE: Removed the bottom Back button as requested */}
    </main>
  );
}
