// src/pages/AdminCoursesContent.jsx
...
export default function AdminCoursesContent() {
  ...
  const backHref = state?.backTo || "/admin-courses"; // 👈 change back route for Admin
          ...
          {Array.isArray(course.lessons) && course.lessons.length > 0 ? (
            <>
              ...
              <ul className="mt-2 list-disc pl-5 text-slate-700">
                {(resolvedLessons.length > 0 ? resolvedLessons : course.lessons).map((item, i) => {
                  const disp = lessonDisplay(item);
                  return (
                    <li key={disp.id || i}>
                      <Link
                        to={`/admin-lessons-content/${disp.id || ""}`} // 👈 route changed to admin
                        state={
                          typeof item === "object"
