...
export default function Instructorlist_admin() {
  ...
  const adminMenu = [
    ...
    { path: "/admin-student-list", label: "Instructor List", icon: UsersIcon },
  ];
  ...
  // Close the row actions menu when clicking outside
  useEffect(() => {
  ...
  const fetchInstructors = async () => {
    try {
      setLoading(true);
      setError("");
      const token = sessionStorage.getItem("token"); // add if your API is protected
  ...
  // Client-side search filter (first/last/email/username)
              ...
              {filtered.map((t) => (
                <tr key={t._id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 relative">
                    <button
                      onClick={() => setMenuOpenId(menuOpenId === t._id ? null : t._id)}
                    ...
                    {menuOpenId === t._id && (
                      <div
                        ref={menuRef}
                        className="absolute right-4 mt-1 w-24 bg-white border border-slate-200 rounded shadow-md z-10"
                      >
                        <button
                          className="w-full text-left px-3 py-1 text-red-600 hover:bg-red-50"
